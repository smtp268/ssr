...
installNewVer() {
    ...
    if [[ $? -ne 0 ]]; then
        echo
        echo -e " [${RED}错误${PLAIN}]: $OS Shadowsocks-libev 安装失败！ 请到 https://t.me/Tony_Chat_bot 反馈"
    ...
    [[ "$ssPath" != "" ]] || {
        cd ${BASE} && rm -rf shadowsocks-libev*
        colorEcho $RED " SS安装失败，请到 https://t.me/Tony_Chat_bot 反馈"
        exit 1
    }
...
menu() {
    clear
    echo "#############################################################"
    echo -e "# ${YELLOW}✅ Shadowsocks/SS install 😄${PLAIN}       #"
